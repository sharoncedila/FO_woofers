...
class AccountService {
  ...
  Future<List<AllUsers>> retrieveAllUsers() async {
    try {
      ...
      var response = await dio.get(api);
      final errorSchema =
          AllUsers.fromJson(response.data['errorSchema']);
      if (errorSchema.errorCode != 'WOF-000') {
        return [AllUsers.fromJson(response.data['errorSchema'])];
      } else {
        return (response.data['outputSchema']['feedsContent'] as List)
            .map((e) => AllUsers.fromJson(e))
            .toList();
      }
    } catch (error) {
